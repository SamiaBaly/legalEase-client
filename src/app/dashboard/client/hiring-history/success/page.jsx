  ...
  try {
    if (sessionId) {
      ...
      const paymentPayload = {
        sessionId: sessionId,
        hireId: hireId, // hireId এখানে যোগ করলাম
      ...
      if (session?.payment_status === 'paid' && hireId) {
        await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/hires/${hireId}/paid`, {
          method: "PATCH",
        });
      }
    }
  ...
  return (
    <div className="min-h-screen bg-[#0b0b10] flex items-center justify-center px-4">
      <div className="w-full max-w-xl text-center">
        ...
        <h1 className="text-white text-3xl mt-4">Payment Successful</h1>
        <div className="mt-6 flex gap-3 justify-center">
          <Link href="/dashboard/client/hiring-history" className="bg-cyan-500 px-6 py-3 rounded-xl text-white">
            Dashboard
          </Link>
        </div>
      </div>
